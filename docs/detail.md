...
- 리스트와 상세가 같은 글을 서로 다른 상태로 보여줄 수 있는 문제
- 승인자가 둘 이상 생기거나 완료 뒤 데이터가 덜 바뀔 수 있는 문제
- 후기 버튼 노출만으로는 권한을 충분히 막기 어려운 문제

분양은 게시판처럼 보여도 실제로는 상태 전이와 권한이 같이 움직이는 기능이었습니다. 그래서 화면보다 먼저 상태 기준과 완료 처리 순서를 어디서 책임질지부터 정리했습니다.
