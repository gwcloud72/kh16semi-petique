...
<form action="edit" method="post" enctype="multipart/form-data">
  ...
  <div class="container w-800">
	  ...
	  <select name="boardAnimalHeader" class="field w-100 mt-2" required>
	      <option value="">-- 분류 선택 --</option>
             <c:forEach var="animalHeader" items="${animalList}">
			  	 <option value="${animalHeader.headerNo}" ${animalHeader.headerNo == boardDto.boardAnimalHeader ? 'selected' : ''}>${animalHeader.headerName}</option>
			 </c:forEach>
		</select>
    ...
    <option value="${typeHeader.headerNo}" ${typeHeader.headerNo == boardDto.boardTypeHeader ? 'selected' : ''}>${typeHeader.headerName}</option>
</c:if>
</c:forEach>
		</select>
